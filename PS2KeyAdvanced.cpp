...
if( retdata > 0 )
  {
  if(retdata == PS2_KEY_CAPS)// || retdata == PS2_KEY_NUM || retdata == PS2_KEY_SCROLL)
    ...
    if( PS2_keystatus & _BREAK )
      ...
      if( PS2_lockstate[ retdata ] == 1 )
        ...
        if( PS2_led_lock & index )
          ...
          PS2_led_lock |= index;
        //set_lock( ); //zawiesza się program po szybkich naciścięciach capsa
        }
      }
    }
    else
    if( retdata == PS2_KEY_L_SHIFT || retdata == PS2_KEY_R_SHIFT ||  retdata == PS2_KEY_L_CTRL || retdata == PS2_KEY_R_CTRL || retdata == PS2_KEY_L_ALT || retdata == PS2_KEY_R_ALT || retdata == PS2_KEY_L_GUI || retdata == PS2_KEY_R_GUI)
